...
def _group_labels(group, side="row"):
    """Display labels for a pivot group, falling back to its raw key values.

    Keys stay ids so groups match correctly; only what is *written* to a cell
    uses the label, otherwise a many2one column exports as a database id.
    """
    labels = group.get(f"{side}Labels")
    if labels:
        return list(labels)
    return [str(v) for v in group.get(f"{side}Values", [])]
...
class SpreadsheetXlsxExporter:
    ...
    def _write_row_pivot(self, ws, groups, row_dims, measure_specs, start_row):
        ...
        row_groups = sorted(
            ...
            key=lambda g: _group_labels(g, "row"),
        )
        totals = [g for g in groups if not g["rowGroupBy"] and not g["colGroupBy"]]
        ...
        for g in row_groups:
            for ci, v in enumerate(_group_labels(g, "row"), 1):
    ...
    def _write_crosstab(self, ws, groups, row_dims, col_dims, measure_specs, start_row):
        ...
        col_groups = sorted(
            ...
            key=lambda g: _group_labels(g, "col"),
        )
        col_keys = [tuple(g["colValues"]) for g in col_groups]
        col_labels = [_group_labels(g, "col") for g in col_groups]
        ...
        r = self._write_crosstab_headers(
            ws, row_dims, col_keys, col_labels, measure_specs, total_col, start_row
        )
    ...
    def _write_crosstab_headers(
        self, ws, row_dims, col_keys, col_labels, measure_specs, total_col, r
    ):
        ...
        for ki in range(len(col_keys)):
            parts = col_labels[ki] if ki < len(col_labels) else None
            label = " / ".join(parts) if parts else _("(none)")
    ...
    def _write_crosstab_data(
        ...
        for rg in row_groups:
            ...
            for ci, v in enumerate(_group_labels(rg, "row"), 1):
                ws.cell(row=r, column=ci, value=v)
