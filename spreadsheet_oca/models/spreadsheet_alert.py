...
class SpreadsheetAlert(models.Model):
    ...
    user_id = fields.Many2one(
        ...
        tracking=True,
        help="The alert is evaluated with this user's permissions, so it can "
        "only ever report on data they are allowed to read.",
    )
    ...
    def _cron_evaluate_all(self):
        ...
        for alert in alerts:
            try:
                alert.with_user(alert.user_id)._evaluate()
            except Exception:
                _logger.exception(
    ...
    def _evaluate(self):
        ...
        if value is None:
            # Not debug: an alert whose cell cannot be read never fires, and
            # silence is indistinguishable from "threshold not crossed".
            _logger.warning(
                "Alert %s (%s): cell %s on sheet %s is missing or non-numeric — "
                "this alert cannot fire",
                self.id,
                self.name,
                self.cell_ref,
                self.sheet_name or "(first)",
            )
            # sudo: bookkeeping. The alert is evaluated as its own user, who
            # only needs *read* rights on the data — writing the evaluation
            # state back is the module's own housekeeping, not a user edit.
            self.sudo().write({"last_checked": fields.Datetime.now()})
            return
        ...
        # sudo: see _evaluate's other write — evaluation state is housekeeping.
        self.sudo().write(
            {
