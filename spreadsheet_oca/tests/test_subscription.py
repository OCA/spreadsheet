...
class TestSpreadsheetSubscription(TransactionCase):
    ...
    def test_defaults_to_the_creating_user(self):
        self.assertEqual(self._make_subscription().user_id, self.env.user)

    def test_cron_sends_each_digest_as_its_own_user(self):
        """The digest must be computed with the subscription's user.

        The shared cron runs as whoever installed the module. Without this
        binding every digest would be computed with those rights and emailed
        out, potentially exposing figures the owner cannot read.
        """
        limited = self.env["res.users"].create(
            {
                "name": "Limited Digest User",
                "login": "limited_digest_user",
                "groups_id": [(6, 0, [self.env.ref("base.group_user").id])],
            }
        )
        limited.email = "limited.digest@example.com"

        sub = self._make_subscription()
        sub.user_id = limited

        seen = []
        original = type(sub)._send_digest_if_due

        def _spy(self_, now=None):
            seen.append((self_.id, self_.env.user.id))
            return original(self_, now)

        self.patch(type(sub), "_send_digest_if_due", _spy)
        self.env["spreadsheet.subscription"]._cron_send_digests()

        runs = [uid for sub_id, uid in seen if sub_id == sub.id]
        self.assertEqual(
            runs,
            [limited.id],
            "the digest must be computed as the subscription's user",
        )
        # The cron swallows per-subscription errors, so prove it completed.
        self.assertTrue(sub.last_sent, "digest ran but could not record last_sent")
