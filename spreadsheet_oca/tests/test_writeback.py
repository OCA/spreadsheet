...
class TestWritebackValueRoundTrip(TransactionCase):
    """old_value is stored as text, so rolling back must restore the type."""

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.spreadsheet = cls.env["spreadsheet.spreadsheet"].create(
            {"name": "Round Trip Sheet", "writeback_enabled": True}
        )
        cls.partner = cls.env["res.partner"].create(
            {"name": "Round Trip Partner", "is_company": False}
        )

    def _log(self, field_name, old_value):
        return self.env["spreadsheet.writeback.log"].create(
            {
                "spreadsheet_id": self.spreadsheet.id,
                ...
                "field_name": field_name,
                "old_value": old_value,
                "new_value": "irrelevant",
            }
        )

    def test_rollback_restores_false_boolean(self):
        """str(False) is "False", which is truthy — it must not roll back True."""
        self.partner.is_company = True
        log = self._log("is_company", str(False))

        self.spreadsheet.action_rollback_writeback(log.id)

        self.assertFalse(
            self.partner.is_company,
            "a boolean that was False must roll back to False, not True",
        )

    def test_rollback_restores_true_boolean(self):
        self.partner.is_company = False
        log = self._log("is_company", str(True))
        self.spreadsheet.action_rollback_writeback(log.id)
        self.assertTrue(self.partner.is_company)

    def test_rollback_restores_numeric_types(self):
        original = self.partner.color
        self.partner.color = original + 3
        log = self._log("color", str(original))
        self.spreadsheet.action_rollback_writeback(log.id)
        self.assertEqual(self.partner.color, original)

    def test_relational_fields_are_not_writeback_candidates(self):
        """A many2one cannot survive a str() round-trip, so it is excluded."""
        from ..models.spreadsheet_writeback import WRITEBACK_FIELD_TYPES

        for fname in ("parent_id", "category_id", "image_1920"):
            field = self.env["res.partner"]._fields[fname]
            self.assertNotIn(
                field.type,
                WRITEBACK_FIELD_TYPES,
                f"{fname} ({field.type}) must not be writeback-eligible",
            )
        self.assertIn(
            self.env["res.partner"]._fields["name"].type, WRITEBACK_FIELD_TYPES
        )
