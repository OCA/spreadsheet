...
These tests verify that get_pivot_data() produces the same grouping
...
from ..models.pivot_data import _dimension_to_groupby, _sections, get_pivot_data
...
class TestPivotData(TransactionCase):
    ...
    def test_many2one_groups_carry_display_labels(self):
        """A many2one group must expose its name, not just its database id.

        rowValues stays an id because grouping needs it, but anything that
        renders a group for a human reads rowLabels — without it a Chatter
        summary shows "13" where it should show "Belgium".
        """
        result = self._run(
            [{"fieldName": "country_id"}], [], [{"fieldName": "__count"}]
        )
        country_groups = self._groups_for(result, ["country_id"], [])

        labels = {g["rowLabels"][0] for g in country_groups}
        self.assertEqual(
            labels, {self.country_be.display_name, self.country_us.display_name}
        )
        for group in country_groups:
            self.assertIsInstance(group["rowValues"][0], int)
